...
"""
# 暗示使用频繁，将其保存在cpu的寄存器中
@register('liif_with_condition')
class LIIF_with_condition(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None,
        ...
        if imnet_spec is not None:
            # condition_in_dim是条件变量
            condition_in_dim = self.encoder.out_dim
            ...
            if self.cell_decode:
                ...
                imnet_in_dim += 2
            self.imnet = models.make(imnet_spec, args={'in_dim': imnet_in_dim , 'condition_dim': condition_in_dim})
        else:
            self.imnet = None
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                q_feat = F.grid_sample(
                    ...
                    mode='nearest', align_corners=False)[:, :, 0, :] \
                    .permute(0, 2, 1)
                # 刚出来结果是[16,576,1,2304]
                # q_feat.shape [16, 2304, 576]
                # 以下是求给区域中心对应坐标
                q_coord = F.grid_sample(
                    feat_coord, coord_.flip(-1).unsqueeze(1),
                    mode='nearest', align_corners=False)[:, :, 0, :] \
                    .permute(0, 2, 1)
                # q_coord.shape [16,2304,2]
                # 相对位置坐标
                rel_coord = coord - q_coord
                # 不知道下面这步骤在干啥
                rel_coord[:, :, 0] *= feat.shape[-2]
                rel_coord[:, :, 1] *= feat.shape[-1]
                inp_condition = q_feat
                # 输入只有坐标
                inp = rel_coord
                ...
                bs, q = coord.shape[:2]
                # 然后还原成一列
                # 输入imnet的结构是[16*2304,580]
                # 相当于每个像素点，预测，张数*每张像素点=N
                pred = self.imnet(latent = inp_condition.contiguous().view(bs * q, -1),coors = inp.view(bs * q, -1)).view(bs, q, -1)
                preds.append(pred)
