...
class SIREN_res(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_list,first_omega_0=30, hidden_omega_0=30.):
        ...
        layers_body = []
        lastv = in_dim
        self.head = SineLayer(lastv, hidden_list[0],is_first=False, omega_0=hidden_omega_0)
        lastv = hidden_list[0]
        for hidden in hidden_list[1:]:
            layers_body.append(nn.Linear(lastv, hidden))
            layers_body.append(nn.ReLU())
            lastv = hidden
        self.body = nn.Sequential(*layers_body)
        layers_tail = []
        layers_tail.append(SineLayer(lastv, hidden_list[0],is_first=False, omega_0=hidden_omega_0))
        for hidden in hidden_list[1:]:
            layers_tail.append(nn.Linear(lastv, hidden))
            layers_tail.append(nn.ReLU())
            lastv = hidden
        layers_tail.append(nn.Linear(lastv, out_dim, bias=True))
        self.tail = nn.Sequential(*layers_tail)


    def forward(self, x):
        shape = x.shape[:-1]
        x = self.head(x.view(-1, x.shape[-1]))
        res = self.body(x)
        res += x
        x = self.tail(res)
